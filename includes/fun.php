...
*return $string
*/
function msg($message,$url="javascript:window.history.back(-1);",$is_time=1,$break='true',$tpl='template/message.html'){
...
//错误信息
function err($message,$url="javascript:history.go(-1);",$break='true',$tpl='template/message.html'){
...
//生成html提示
function show_htm($message,$url="javascript:history.go(-1);",$is_time=1,$break='true',$tpl='template/show_htm.html'){
	$message="<p>".$message."</p>";
...
function form_fields($id,$path='',$edit='false'){
	...
	$opt='';
		foreach($field as $key=>$value){
			...
			if($value['form_id']==$id){
				...
				if(!in_array($value['field_type'],array('checkbox','radio'))){
				$help=empty($value['info'])?"":"<span title=\"{$value['field_info']}\" class=\"help\"></span>";
				$opt.="<tr><td class=\"w1\" id=\"w30\">{$help}{$value['use_name']}:</td><td id=\"w70\">".$value['field_type']($value['field_name'],$v,1)."{$note}</td></tr>";
				}else{
				if($edit=='false'){$v='';}
				$opt.="<tr><td class=\"w1\" id=\"w30\"><span title=\"{$value['field_info']}\" class=\"help\"></span>{$value['use_name']}:</td><td id=\"w70\">".$value['field_type']($value['field_name'],$value['field_value'],$v)."{$note}</td></tr>";
				}
			}
		}
	...
	$str.=$opt;	
	$str.="<tr><td id=\"w30\" style=\"text-align:right\" class=\"w1\">".$GLOBALS['language']['code']."</td><td id=\"w70\" style=\"text-align:left\"><input name=\"feed_code\" value=\"\" style=\"width:50px; height:20px; line-height:20px; display:block; float:left; margin-right:3px; display:inline\"/><img src=\"".CMS_SELF."includes/code.php\" name=\"code\" border=\"0\" id=\"form_code\" style=\"display:block; float:left; cursor:pointer\"/>&nbsp;<em id=\"refresh_code\">".$GLOBALS['language']['code_info']."</em></td></tr>";
	$str.="<tr><td id=\"w30\"></td><td id=\"w70\"><input type=\"hidden\" value=\"{$lang}\" name=\"lang\"/><input type=\"hidden\" value=\"{$f_id}\" name=\"f_id\"/><input type=\"submit\" name=\"submit\" class=\"pt_go\" value=\"提交\"/><input type=\"reset\" name=\"reset\" class=\"pt_reset\" value=\"重置\"/></td></tr>";
...
function page($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	global $language,$_confing,$php_file_copy;
	$str="";
	$op="";
	...
	if($_confing['web_html'][0]&&!$ishtml){
		...
		$str.=($page>1)?'<a href="'.CMS_URL.$cate_info['cate_fold_name'].'.html">'.$language['pagehome'].'</a>':'<span class="off">'.$language['pagehome'].'</span>';
		if($page>1){
			
			if(($page-1)==1){
				$str.='<a class="p" href="'.CMS_URL.$cate_info['cate_fold_name'].'.html">'.$language['pagapre'].'</a>';
			}else{
				$str.='<a class="p" href="'.CMS_URL.$cate_info['cate_fold_name'].'-'.$pre_page.'.html">'.$language['pagapre'].'</a>';
			}
		}else{
		...
		for($i=$page_star;$i<=$page_end;$i++){
			...
			if($i==1){
				$str.='<a class="'.$class.'" href="'.CMS_URL.$cate_info['cate_fold_name'].'.html">1</a>';
			}else{
				$str.='<a class="'.$class.'" href="'.CMS_URL.$cate_info['cate_fold_name'].'-'.$i.'.html">'.$i.'</a>';
			}
			if($i==$page_end&&$i!=$totalpages){$str.='...';}
		}
		
		for($i=1;$i<=$totalpages;$i++){
			$op.=($i==1)?"<option value=\"index.html\" {$sel}>{$i}</option>":"<option value=\"".$cate_info['cate_fold_name']."-{$i}.html\" {$sel}>{$i}</option>";
		}
		
		if($page<$totalpages){
			$str.='<a class="p" href="'.CMS_URL.$cate_info['cate_fold_name'].'-'.$next_page.'.html">'.$language['pagenext'].'</a>';
		}else{
			$str.='<span class="off">'.$language['pagenext'].'</span>';
		}
		$str.=($page<$totalpages)?'<a href="'.CMS_URL.$cate_info['cate_fold_name'].'-'.$totalpages.'.html">'.$language['pageend'].'</a>':'<span class="off">'.$language['pageend'].'</span>';
...
//取得位置
function get_position($parent,$cate,$path,$list_php=''){
...
function pw(){
	return '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';
}
function get_sitemap_child($id){
	...
	if(!empty($child)){
		foreach($child as $row){
			...
			$url=(!$row['is_html']&&!($GLOBALS['_confing']['web_html'][0]))?$path.$list_php.'?id='.$cate_info['id']:$path.$cate_info['cate_fold_name'].'.html';
			if($row['cate_tpl']==3){
				$url=(!$row['is_html']&&!($GLOBALS['_confing']['web_html'][0]))?$path.$list_php.'?id='.$cate_info['id']:$path.$cate_info['cate_fold_name'].'.html';
			}
			$rel[$row['id']]['url']=$url;
	...
	foreach($rel as $row){
		if(!$row['cate_hide']){
			...
			if($row['cate_tpl']==2){
				...
				$url=($row['cate_html']&&$_confing['web_html'][0])?CMS_URL.$row['cate_fold_name'].'.html':CMS_URL.$list_php.'?id='.$row['id'];
			if($row['cate_tpl']==3){
				$url=($row['cate_html']&&$_confing['web_html'][0])?CMS_URL.$row['cate_fold_name'].'.html':CMS_URL.$list_php.'?id='.$row['id'];
			}
			}
