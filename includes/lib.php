...
if(!empty($_confing)){
 foreach($_confing as $k=>$v){
	...
	if($GLOBALS['mysql']->fetch_rows($sql)){
		...
		$url=(!empty($next_rel[0]['is_html'])||!($_confing['web_html'][0]))?$path.$content_php.'?id='.$prev_rel[0]['id']:$path.$cate_info['cate_fold_name'].'/'.$html_url.'.html';
	...
	if($GLOBALS['mysql']->fetch_rows($sql)){
		...
		$html_url = empty($next_rel[0]['custom_url'])?date('YmdHms',$next_rel[0]['addtime']):$next_rel[0]['custom_url'];		
		$url=(!empty($next_rel[0]['is_html'])||!($_confing['web_html'][0]))?$path.$content_php.'?id='.$next_rel[0]['id']:$path.$cate_info['cate_fold_name'].'/'.$html_url.'.html';
		$next_is=isset($next_rel[0]['is_url'])?$next_rel[0]['is_url']:'';
		$url=($next_is)?$next_rel[0]['url_add']:$url;
		$font_style=empty($font_style)?'':'style="'.$font_style.'"';
 ...
 */
function body_pages(){
	global $id,$content_arr_num,$content_focus,$arc_id,$lang,$cat_id,$category;//程序页show_content.php的值
	$_confing=get_confing($lang);
	if(empty($id)){return;}
	$str='';
	...
	if($_confing['web_html'][0])
	{
		...
		$url = empty($rel[0]['custom_url'])?CMS_URL.$cate_info['cate_fold_name'].'/'.date('YmdHms',$rel[0]['addtime']):CMS_URL.$cate_info['cate_fold_name'].'/'.$rel[0]['custom_url'];	
		/*
		if(!empty($rel[0]['url']))
		{
		...
		{//静态html分页
			for($i=0 ; $i<$content_arr_num ; $i++)
			{
				...
				if($content_focus == $i)
				{
					...
					{
						$str_pre=($c_page<=0)?'<span class="off">'.$GLOBALS['language']['pagapre'].'</span>':'<a href="'.$url.'-'.$c_page.'.html">'.$GLOBALS['language']['pagapre'].'</a>';	
					}
					$c_page=$p+1;
					$str_next=($p>=$content_arr_num)?'<span class="off">'.$GLOBALS['language']['pagenext'].'</span>':'<a href="'.$url.'-'.$c_page.'.html">'.$GLOBALS['language']['pagenext'].'</a>';
				}
				$str.=($i==0)?"<a {$class} href=\"{$url}.html\">{$p}</a>":"<a {$class} href=\"{$url}-{$p}.html\">{$p}</a>";
...
function position(){
	...
	if(get_index_url($GLOBALS['lang'])){
		...
		$index_str=$_confing['web_html'][0]?CMS_URL.'index_'.$GLOBALS['lang'].'.html':CMS_URL.'index.php?lang='.$GLOBALS['lang'];
	}
	if(!empty($cat_id)){
		$path=CMS_URL;
		...
		if(!empty($cate_list)){
			foreach($cate_list as $k=>$v){
				if($v['id']==$cat_id){
					...
					$is_dy=($v['cate_channel']==1)?$v['cate_fold_name'].'.html':$v['cate_fold_name'].'.html';//单页重新设置
					$url=($v['cate_html']&&$_confing['web_html'][0])?CMS_URL.$is_dy:CMS_URL.$list_php."?id={$v['id']}";
	...
	if(!empty($cate_list)){
			foreach($cate_list as $k=>$v){
				if($cate==$v['id']){
					...
					$arr[0]['url']       = (($v['cate_html'])&&($GLOBALS['_confing']['web_html'][0]))?$path.$v['cate_fold_name'].'.html':$path.$list_php.'?id='.$v['id'];
					if($v['cate_tpl']==3){
						$arr[0]['url']   = ($v['cate_html']&&$GLOBALS['_confing']['web_html'][0])?$path.$v['cate_fold_name'].'.html':$path.$list_php.'?id='.$v['id'];
					}
				}
...
function nav_middle(){
	...
	if(!empty($cate_list)){
		...
		if(in_array('2',$cate_nav)){
			...
			if($v['cate_tpl']==2){
				...
				$url=($v['cate_html']&&$_confing['web_html'][0])?$path.$v['cate_fold_name'].'.html':$path.$list_php.'?id='.$v['id'];
				if($v['cate_tpl']==3){
					$url=($v['cate_html']&&$_confing['web_html'][0])?$path.$v['cate_fold_name'].'.html':$path.$list_php.'?id='.$v['id'];
				}
	...
	if($is_book){
		...
		if($is_main_lang)
		{
			$book['url']=CMS_URL.'book/book.php';
		}
		else
		{
			$book['url']=CMS_URL.'book/book.php?lang='.$lang;
		}
...
function nav_bottom(){
	...
	if($is_main_lang)
	{
		$map_arr['url']=$_confing['web_html'][0]?$path.$lang.'_sitemap.html':$path.'sitemap/sitemap.php';
	}
	else
	{
		$map_arr['url']=$_confing['web_html'][0]?$path.$lang.'_sitemap.html':$path.'sitemap/sitemap.php?lang='.$lang;
	}
...
foreach($rel_search as $k=>$v){
	...
	$html_url = empty($v['custom_url'])?date('YmdHms',$v['addtime']):$v['custom_url'];//内容静态页面	
	if($v['purview']){
		$url=$path.'show_content.php?id='.$v['id'];
	}else{
		$url=(!$v['is_html']&&!($GLOBALS['_confing']['web_html'][0]))?$path.$content_php.'?id='.$v['id']:$path.$cate_info['cate_fold_name'].'/'.$html_url.'.html';
	}	
 ...
 */
 function get_tpl_cate_content($tpl_id='',$limit='0,5',$order_type='id',$filter='',$pic='no',$order='desc',$lang=''){
	...
	if($cate_rel[0]['cate_tpl']==2){
		...
		$cate_url=($cate_rel[0]['cate_html']&&$_confing['web_html'][0])?CMS_URL.$cate_rel[0]['cate_fold_name'].'.html':$path.$list_php.'?id='.$cate_rel[0]['id'];
			if($c_v['cate_tpl']==3){
				$cate_url=($cate_rel[0]['cate_html']&&$_confing['web_html'][0])?CMS_URL.$cate_rel[0]['cate_fold_name'].'.html':$path.$list_php.'?id='.$cate_rel[0]['id'];
			}
	}
	...
	if(!empty($cate_rel)){
		foreach($cate_rel as $c_v){
			...
			if($c_v['cate_tpl']==2){
				...
				$cate_url=($c_v['cate_html']&&$_confing['web_html'][0])?CMS_URL.$c_v['cate_fold_name'].'.html':$path.$list_php.'?id='.$c_v['id'];
				if($c_v['cate_tpl']==3){
					$cate_url=($c_v['cate_html']&&$_confing['web_html'][0])?CMS_URL.$c_v['cate_fold_name'].'.html':$path.$list_php.'?id='.$c_v['id'];
				}
			}
	...
	if(!empty($cate_rel)){
		foreach($cate_rel as $c_v){
			...
			if($v['cate_tpl']==2){
				...
				$cate_url=($c_v['cate_html']&&$_confing['web_html'][0])?CMS_URL.$c_v['cate_fold_name']:$path.$list_php.'?id='.$c_v['id'];
				if($c_v['cate_tpl']==3){
					$cate_url=($c_v['cate_html']&&$_confing['web_html'][0])?CMS_URL.$c_v['cate_fold_name']:$path.$list_php.'?id='.$c_v['id'];
				}
			}
